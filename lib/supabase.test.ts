...
// insert/update/delete/upsert/rpc — if fetchPingsInRange or
// checkDatabaseConnectivity ever grew a write call, this mock would throw
// "not a function" rather than silently succeeding, which is the regression
// guard for the "read-only by construction" guarantee documented in
// lib/supabase.ts.
...
describe("checkDatabaseConnectivity", () => {
  ...
  it("reports row count without exposing row contents", async () => {
    const { builder } = makeQueryBuilder({ count: 4821, error: null });
    mockFrom.mockReturnValue(builder);

    expect(await checkDatabaseConnectivity()).toBe("ok (4821 rows)");
  });

  it("reports the error message when the connectivity check query fails", async () => {
    const { builder } = makeQueryBuilder({ count: null, error: { message: "relation does not exist" } });
    mockFrom.mockReturnValue(builder);

    expect(await checkDatabaseConnectivity()).toBe("error: relation does not exist");
  });
